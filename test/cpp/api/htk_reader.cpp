...
void HTKParser::parse_file_set(const string &file_set_name)
{
    ...
    // TODO: should we change this to txt file?
    ptree doc;
...
extern std::string c_dataset_worker;
extern std::string c_dataloader_worker;
std::string dataSetWorker = "datasetWorker=";
std::string dataLoaderWorker = "dataloaderWorker=";

TEST(DataTest, HTKChunkDatasetLargeData) {
  ASSERT_EQ(c_dataset_worker.substr(0, dataSetWorker.size()).compare(dataSetWorker), 0);
  ASSERT_EQ(c_dataloader_worker.substr(0, c_dataloader_worker.size()).compare(c_dataloader_worker), 0);

  std::string str_dataset = c_dataset_worker.substr(dataSetWorker.size());
  std::string str_dataloader = c_dataloader_worker.substr(dataLoaderWorker.size());

  int dataset_worker = std::stoi(str_dataset);
  int dataloader_worker = std::stoi(str_dataloader);
