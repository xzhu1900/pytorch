...
struct ChunkDatasetOptions {
  ...
  // no cross-chunk shuffling.
  // Note: there is a performance penalty when cross_chunk_shuffle_count is
  // greater than 1, as we need to merge multiple chunks into one before
  // performing example sampling. Usually the default config (1 chunk shuffle
  // + example shuffle) is good enough to genreate random distributed data. Use
  // this parameter only if you know cross-shuffle is needed in your case.
